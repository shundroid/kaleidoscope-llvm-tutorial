...
static std::unique_ptr<ExprAST> ParseVarExpr() {
  ...
  if (CurTok != tok_in) return LogError("expected 'in' after 'var' definitions");
  getNextToken();
...
static std::unique_ptr<ExprAST> ParseForExpr() {
  ...
  if (CurTok != '=') return LogError("expected '=' after identifier");
  getNextToken();
...
Value *VarExprAST::codegen() {
  ...
  for (unsigned i = 0, e = VarNames.size(); i != e; ++i) {
    NamedValues[VarNames[i].first] = OldBindings[i]; // todo: maybe fast using next
  }
...
Value *IfExprAST::codegen() {
  ...
  // InsertPoint may change after codegen
...
Function *FunctionAST::codegen() {
  ...
  // maybe an extern function
  Function *TheFunction = getFunction(P.getName());
  // if (!TheFunction) TheFunction = Proto->codegen();
  if (!TheFunction) return nullptr;

  // dynamically update the parser!
  if (P.isBinaryOp())
  ...
  if (Value *RetVal = Body->codegen()) {
    ...
    // provided from LLVM. catch a lot of bugs
